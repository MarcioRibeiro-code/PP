...
public class EventManager_ implements EventManager {
    ...
    public void reportEvent(Event event) throws EventManagerException {
        ...
        try {
            // Event_toJSON
            InsuranceReporter.addEvent(Event_toJSON(this.Event[this.getEvent().length - 1]));
            //this.Event[this.getEvent().length - 1]

            if (event instanceof Accident_) {
                Accident_ temp_event = (Accident_) this.Event[this.getEvent().length - 1];
                temp_event.setInsurance_Notified(true);
            } else if (event instanceof Failure_) {
                Failure_ temp_event = (Failure_) this.Event[this.getEvent().length - 1];
                temp_event.setInsurance_Notified(true);
            } else if (event instanceof Incident_) {
                Incident_ temp_event = (Incident_) this.Event[this.getEvent().length - 1];
                temp_event.setInsurance_Notified(true);
            }
